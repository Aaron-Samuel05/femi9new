 ...
 * Client-only, persists to localStorage key `femi9:tracker`.
 * Two states: SETUP (no saved data) and RESULT (saved data present).
 * All date math is local-midnight based to avoid TZ / NaN surprises.
 * ------------------------------------------------------------------ */

const STORAGE_KEY = 'femi9:tracker'
...
function load(): TrackerData | null {
  try {
    ...
    if (o && typeof o.lastPeriod === 'string' && parseISO(o.lastPeriod)) {
      return {
        lastPeriod: o.lastPeriod,
        cycleLength: clampInt(o.cycleLength, CYCLE_MIN, CYCLE_MAX, CYCLE_DEFAULT),
        periodLength: clampInt(o.periodLength, PERIOD_MIN, PERIOD_MAX, PERIOD_DEFAULT),
      }
    }
  } catch {
    /* private-mode / quota / malformed — fall through to setup */
  }
  return null
}
function save(data: TrackerData) {
  try {
    ...
    /* ignore — the in-memory result still renders */
  }
}
...
export function CycleTracker() {
  ...
  const [data, setData] = useState<TrackerData | null>(() => load())
  const [mode, setMode] = useState<'setup' | 'result'>(() => (load() ? 'result' : 'setup'))

  // form state (prefilled from saved data when present)
  const [lastPeriod, setLastPeriod] = useState(() => data?.lastPeriod ?? '')
  const [cycleLength, setCycleLength] = useState(() => data?.cycleLength ?? CYCLE_DEFAULT)
  const [periodLength, setPeriodLength] = useState(() => data?.periodLength ?? PERIOD_DEFAULT)
        ...
        <div className={`cyc-panel${mode === 'result' ? ' is-result' : ''}`}>
          ...
          <div className="cyc-intro">
            ...
            <p className="cyc-lead">
              {mode === 'setup'
                ? 'A gentle, private tracker that lives right here in your browser. No login, no sharing. Tell us three things and see when your next period is likely to arrive.'
                : 'Here is your rhythm at a glance. It stays on this device only, so you can plan your days and your Femi9 pack with a little more calm.'}
            </p>
            <ul className="cyc-assurances" aria-label="How this tracker treats your data">
              <li>Stays on your device. Nothing leaves your browser.</li>
              <li>Awareness first, so no month catches you by surprise.</li>
              <li>Edit or clear it any time.</li>
            </ul>
          </div>
