...
function href(brand: string, { status, q, page }: { status?: string; q?: string; page?: number }) {
  const p = new URLSearchParams()
  if (status) p.set('status', status)
  if (q) p.set('q', q)
  if (page && page > 1) p.set('page', String(page))
  const s = p.toString()
  return `/${brand}/orders` + (s ? `?${s}` : '')
}

export default async function OrdersPage(props: { params: Promise<{ brand: string }>; searchParams: Promise<Search> }) {
  const { brand } = await requireConsole((await props.params).brand, 'orders')
  const searchParams = await props.searchParams;
  const status = searchParams.status
  const q = searchParams.q?.trim() || undefined
  const page = Number(searchParams.page) || 1

  const { orders, total, page: current, pageCount } = await listOrders(brand, { status, q, page })
  ...
  return (
    <div>
      <div className="adm-toolbar">
        ...
        <div className="adm-chip-group">
          {chips.map((c) => {
            ...
            return (
              <Link
                key={c.label}
                href={href(brand, { status: c.value, q })}
                className="adm-chip"
                aria-pressed={active}
                style={
                  active
        ...
        <form action={`/${brand}/orders`} method="get" className="adm-row" style={{ alignItems: 'center' }}>
      ...
      {orders.length === 0 ? (
        ...
        <>
          ...
          <div className="adm-pager">
            ...
            {current > 1 ? (
              <Link className="adm-btn adm-btn--secondary adm-btn--sm" href={href(brand, { status, q, page: current - 1 })}>
                Previous
              </Link>
            ) : (
