 ...
 * mutates through /<brand>/api/affiliates, reconciling with the rows the server
...
export default function AffiliatesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(false)
    try {
      const res = await fetch(`/${brand}/api/affiliates`, { cache: 'no-store' })
  ...
  async function runAction(row: AffiliateRow, action: 'approve' | 'suspend') {
    ...
    try {
      const res = await fetch(`/${brand}/api/affiliates/${row.id}`, {
        method: 'PATCH',
  ...
  const loadPayouts = useCallback(async (affiliateId: string) => {
    ...
    try {
      const res = await fetch(`/${brand}/api/affiliates?payouts=1&affiliateId=${affiliateId}`, {
        cache: 'no-store',
      })
      if (!res.ok) throw new Error()
  ...
  async function submitPayout(e: React.FormEvent, affiliateId: string) {
    ...
    setPayoutError(null)
    try {
      const res = await fetch(`/${brand}/api/affiliates/${affiliateId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
