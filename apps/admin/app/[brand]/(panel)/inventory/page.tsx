 ...
 * with optimistic updates. It loads the list from GET /<brand>/api/inventory on
...
export default function InventoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(false)
    try {
      const res = await fetch(`/${brand}/api/inventory`, { cache: 'no-store' })
  ...
  const commit = useCallback(
    async (variantId: string, body: { stock: number } | { delta: number }) => {
      ...
      try {
        const res = await fetch(`/${brand}/api/inventory/${variantId}`, {
          method: 'PATCH',
