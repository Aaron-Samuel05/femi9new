 ...
 * interactivity (create / edit / delete via /<brand>/api/pricing-zones).
 *
...
export default async function PricingZonesPage({ params }: { params: Promise<{ brand: string }> }) {
  const { brand } = await requireConsole((await params).brand, 'pricing')
  const [zones, catalog] = await Promise.all([
    listZones(brand) as Promise<ServiceZone[]>,
    listPricingCatalog(brand),
  ])
