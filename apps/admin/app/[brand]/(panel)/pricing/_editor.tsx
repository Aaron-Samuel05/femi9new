 ...
 * deleting, and re-reading the list from GET /<brand>/api/pricing-zones after any
 * mutation.
 *
...
export default function PricingZonesScreen({
  ...
  const load = useCallback(async () => {
    setRefreshing(true)
    try {
      const res = await fetch(`/${brand}/api/pricing-zones`, { cache: 'no-store' })
  ...
  async function onSubmit(ev: React.FormEvent) {
    ...
    const url = form.id ? `/${brand}/api/pricing-zones/${form.id}` : `/${brand}/api/pricing-zones`
  ...
  async function onDelete(z: ZoneRow) {
    ...
    setPendingId(z.id)
    try {
      const res = await fetch(`/${brand}/api/pricing-zones/${z.id}`, { method: 'DELETE' })
