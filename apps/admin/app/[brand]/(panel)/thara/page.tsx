 ...
 *
 * Three surfaces on one page: KPIs from /<brand>/api/thara/metrics, the member
 * roster from /<brand>/api/thara/list (searchable, filterable, with suspend and
...
export default function TharaAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [m, v] = await Promise.all([
        fetch(`/${brand}/api/thara/metrics`, { cache: 'no-store' }).then((r) => r.json()),
        fetch(`/${brand}/api/thara/vouchers?missingCode=true&take=50`, { cache: 'no-store' }).then((r) => r.json()),
      ])
      setMetrics(m)
      setRows(v.rows ?? [])
    } catch {
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/${brand}/api/thara/list?${params}`, { cache: 'no-store' })
  ...
  async function saveCode(id: string) {
    ...
    setActionError(null)
    try {
      const res = await fetch(`/${brand}/api/thara/vouchers/${id}/set-code`, {
        method: 'POST',
  ...
  async function setSuspended(id: string, next: boolean) {
    ...
    setActionError(null)
    try {
      const res = await fetch(`/${brand}/api/thara/${id}/${next ? 'suspend' : 'unsuspend'}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
