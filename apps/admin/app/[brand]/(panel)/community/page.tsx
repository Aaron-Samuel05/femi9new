 ...
 * row in place without a full page reload. It loads from GET /<brand>/api/wall and
...
export default function CommunityPage() {
  ...
  const load = useCallback(async (f: Filter) => {
    ...
    setLoadError(false)
    try {
      const url = `/${brand}/api/wall` + (f ? `?status=${f}` : '')
  ...
  const moderate = useCallback(
    async (row: WallRow, status: ModerationStatus) => {
      ...
      try {
        const res = await fetch(`/${brand}/api/wall/${row.id}`, {
          method: 'PATCH',
  ...
  const remove = useCallback(
    async (row: WallRow) => {
      ...
      setPendingFor(row.id, true)
      try {
        const res = await fetch(`/${brand}/api/wall/${row.id}`, { method: 'DELETE' })
        if (!res.ok) {
          const msg = (await res.json().catch(() => null))?.error as string | undefined
