...
export default async function AdminDashboardPage({ params }: { params: Promise<{ brand: string }> }) {
  const { brand } = await requireConsole((await params).brand, 'dashboard')
  const o = await getOverview(brand)
      ...
      <section className="adm-grid">
        ...
        <div className="adm-card" style={{ padding: 0 }}>
          <div className="adm-card-head" style={{ padding: '18px 20px 0' }}>
            ...
            <Link className="adm-btn adm-btn--ghost adm-btn--sm" href={`/${brand}/inventory`}>
              Manage →
            </Link>
          </div>
