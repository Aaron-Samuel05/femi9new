...
export default async function CustomerDetailPage(props: { params: Promise<{ brand: string; id: string }> }) {
  const { brand } = await requireConsole((await props.params).brand, 'customers')
  const params = await props.params;
  const c = await getCustomer(brand, params.id)
  if (!c) notFound()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <Link href={`/${brand}/customers`} className="adm-btn adm-btn--ghost adm-btn--sm" style={{ paddingLeft: 0 }}>
          ← All customers
        </Link>
      </div>
