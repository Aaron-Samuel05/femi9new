...
export default async function CustomersPage(props: {
  ...
  const { brand } = await requireConsole((await props.params).brand, 'customers')
  const searchParams = await props.searchParams;
  const q = typeof searchParams.q === 'string' ? searchParams.q : ''
  const pageNum = Number(searchParams.page)
  const page = Number.isFinite(pageNum) && pageNum > 0 ? pageNum : 1

  const { items, total, totalPages, pageSize } = await listCustomers(brand, { q, page })
  ...
  const pageHref = (p: number) => {
    ...
    return qs ? `/${brand}/customers?${qs}` : `/${brand}/customers`
  }
  ...
  return (
    <>
      <div className="adm-toolbar">
        ...
        <form method="GET" action={`/${brand}/customers`} className="adm-row" style={{ alignItems: 'flex-end' }}>
          ...
          {q ? (
            <Link href={`/${brand}/customers`} className="adm-btn adm-btn--ghost">
              Clear
            </Link>
          ) : null}
        </form>
