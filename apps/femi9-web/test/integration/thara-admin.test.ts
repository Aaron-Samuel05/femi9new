...
describe('admin thara service', () => {
  ...
  it('suspends an active member and preserves prior status', async () => {
    ...
    const after = await suspendMembership('femi9', id, 'suspected abuse')
  ...
  it('unsuspend restores the prior status', async () => {
    ...
    await suspendMembership('femi9', id, 'x')

    const restored = await unsuspendMembership('femi9', id)
  ...
  it('throws for missing membership', async () => {
    await expect(suspendMembership('femi9', 'does-not-exist', 'x')).rejects.toBeInstanceOf(
      TharaNotFoundError,
    )
    await expect(unsuspendMembership('femi9', 'does-not-exist')).rejects.toBeInstanceOf(
      TharaNotFoundError,
    )
  })
  ...
  it('list paginates and filters by status', async () => {
    ...
    const first = await listMemberships('femi9', { take: 2, skip: 0 })
