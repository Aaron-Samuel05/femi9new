...
describe('attributeReferralIfPresent', () => {
  ...
  it('attaches a TharaReferral when everything checks out', async () => {
    ...
    const result = await attributeReferralIfPresent('femi9', referred, await ctxOf(refMemId))
  ...
  it('rejects self-referral', async () => {
    ...
    const { id: refMemId } = await enrollUser('femi9', u.id, 'v1')
    const result = await attributeReferralIfPresent('femi9', u, await ctxOf(refMemId))
  ...
  it('rejects when the referred and referrer share an email (dup account)', async () => {
    ...
    const result = await attributeReferralIfPresent('femi9', impostor, await ctxOf(refMemId))
  ...
  it('rejects when the referred and referrer share a phone (dup account)', async () => {
    ...
    const result = await attributeReferralIfPresent('femi9', referredForced, await ctxOf(refMemId))
  ...
  it('no-ops when the cookie is missing', async () => {
    const referred = await makeUser()
    const result = await attributeReferralIfPresent('femi9', referred, {
      cookieToken: null,
      ip: null,
      ua: null,
    })
  ...
  it('refuses a second referrer for the same user (one-referrer-per-user)', async () => {
    ...
    const { id: r1Mem } = await enrollUser('femi9', r1User.id, 'v1')
    const { id: r2Mem } = await enrollUser('femi9', r2User.id, 'v1')
    const referred = await makeUser()
    await attributeReferralIfPresent('femi9', referred, await ctxOf(r1Mem))
    const result = await attributeReferralIfPresent('femi9', referred, await ctxOf(r2Mem))
