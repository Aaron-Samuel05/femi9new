 ...
 *
 * Reads reuse getSettings() so the editor and the live storefront always agree
 ...
 * "225"-style string that would only survive because getSettings() coerces it.
 */
...
export async function getEditableSettings(): Promise<Settings> {
  return getSettings()
}
