...
export default async function ProductPage({ params }: { params: Promise<{ size: string }> }) {
  ...
  return (
    <PageShell links={NAV_LINKS}>
      ...
      {/* Customer Reviews \u2014 minimal empty-state layout matching the Femi9
          screenshot. The component handles both states (empty + populated),
          the "Write a review" button, the modal, and the media strip on each
          approved review. See CustomerReviews.tsx for the full shape. */}
      <CustomerReviews productSlug={size.slug} reviews={reviews} />
    </PageShell>
  );
}
