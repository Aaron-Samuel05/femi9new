/**
 * Client-side mandate authorisation — the second half of starting a box plan.
 *
 ...
 *
 * It is deliberately UI-free so both the box builder and the account page can
 * call it — one of them starts a plan, the other finishes one the shopper
 * abandoned, and the gateway dance is identical.
 *
 * A near-copy of femi9-web's `src/lib/mandate.ts`, and kept as one rather than
 * hoisted into `@femi9/core`: this is a CLIENT module (it touches `window` and
 * injects a script tag) and core is `server-only` throughout. The two differ in
 * the brand name Checkout displays, which is exactly the kind of thing that must
 * not be parameterised into a shared file nobody reads.
 */
...
export async function authorizeMandate(
  ...
  return new Promise<MandateOutcome>((resolve) => {
    ...
    const checkout = new Razorpay({
      ...
      name: "Lumi9",
      description: opts?.description ?? 'Diaper subscription',
