 ...
 *
 * SITE-RELATIVE on purpose — no CDN hostname is hardcoded here, exactly as
 * `app/api/admin/upload/route.ts` returns `/uploads/<name>` for product photos.
 * The path resolves differently per environment with no code change:
 *
 ...
 *
 * Baking in a distribution hostname would pin every environment to one bucket,
 * which is the thing that pattern exists to avoid.
 *
 * NOT in /public and therefore not in the repo: 11MB of binary is 11MB in every
 * clone and container image forever, and the app server would be streaming the
 * video itself — including the range requests seeking generates — rather than
 * letting an edge cache do it.
 */
