  ...
  return (
    <header className={`nav${scrolled ? ' scrolled' : ''}`} id="nav">
      <div className="nav-in">
        <Link to="/" className="nav-logo" aria-label="Femi9 home"><img src="/assets/img/logo.png" alt="Femi9" /></Link>
        ...
        <div className="nav-right">
          ...
          <Link to="/dashboard" className="cart-btn" aria-label="My account"><IUser /></Link>
          <button className="cart-btn" onClick={openCart} aria-label="Open bag"><Bag /><span className={`cart-count${count > 0 ? ' show' : ''}`}>{count}</span></button>
